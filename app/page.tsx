...
//       const response = await fetch("https://newsapi.org/v2/everything?q=tesla&from=2024-12-05&sortBy=publishedAt&apiKey=4962b8ca3e3f40f393a3326ce49de7b7")
...
export default function Home() {
  const [loading, setLoading] = useState<boolean>(true);
  const [error, setError] = useState<string>('');
  const { data, setOriginalData } = useApiDataContext();  // Accessing filtered data from context
  ...
  const dataToDisplay = data.length > 0 ? data : []; // Show the filtered data from context, or an empty array if no data
